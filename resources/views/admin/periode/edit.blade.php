...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Edit Periode Pendaftaran</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.periode.update', $periode->id) }}" method="POST">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="tahun_akademik" class="form-control" value="{{ old('tahun_akademik', $periode->tahun_akademik) }}" placeholder="2024/2025" required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="praktikum_ke" class="form-control" value="{{ old('praktikum_ke', $periode->praktikum_ke) }}" min="1" placeholder="Contoh: 1" required>
                    @error('praktikum_ke')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal_awal" class="form-control" value="{{ old('tanggal_awal', $periode->tanggal_awal->format('Y-m-d')) }}" required>
                    @error('tanggal_awal')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal_akhir" class="form-control" value="{{ old('tanggal_akhir', $periode->tanggal_akhir->format('Y-m-d')) }}" required>
                    @error('tanggal_akhir')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group checkbox-group">
                <label class="checkbox-label">
                    <input type="checkbox" name="is_aktif" value="1" {{ old('is_aktif', $periode->is_aktif) ? 'checked' : '' }}>
